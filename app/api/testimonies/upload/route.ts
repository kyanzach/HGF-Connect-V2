...
// POST /api/testimonies/upload — upload testimony photo (→ WebP)
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // We reuse "listing" type for standard webp conversion
    const { buffer } = await processImage(rawBuffer, "listing");

    const filename = `${randomBytes(8).toString("hex")}.webp`;
    const uploadDir = join(process.cwd(), "public", "uploads", "testimonies");
    ...
    console.error("[testimonies/upload]", (err as Error).message);
